    ...
    def encodeBatch(
        ...
        if out == 'latent':
            ...
            for x, y, idx in dataloader:
                x = x.float().to(device)
                z = self.encoder(x)[1]  # z, mu, var
                z_new = torch.randn(z.shape).to(device)
                output[idx] = self.decoder(z_new, torch.LongTensor([batch_id] * len(z))).detach().cpu().numpy()
                indices[idx] = idx
                batch_list.extend(y.detach().cpu().tolist())
            batch_array = np.array(batch_list)
            return output, batch_array
    ...
    def fit(
        ...
        with tqdm(range(n_epoch), total=n_epoch, desc='Epochs') as tq:
            for epoch in tq:
                ...
                # # 多个epoch
                # if epoch % 4 == 0:
                #     adata.obsm[f'latent_epoch{epoch}'] = self.encodeBatch(testloader, device=device)
